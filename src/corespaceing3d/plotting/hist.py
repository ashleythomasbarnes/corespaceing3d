...
def plot_length_histograms(ax, len3: np.ndarray, len2: np.ndarray, bins=20):
    ...
    # Plot histograms
    ax.hist(
        len3, bins=bins, color="tab:blue", alpha=0.4, edgecolor="black", density=True, label="3D"
    )
    ax.hist(
        len2, bins=bins, color="tab:orange", alpha=0.4, edgecolor="black", density=True, label="2D"
    )
    ...
    ax.text(
        0.98,
        0.84,
        r"$\langle \ell_{\rm 3D} \rangle/ \langle \ell_{\rm 2D} \rangle$ =" f"{mean3 / mean2:.2f}",
        transform=ax.transAxes,
        ha="right",
        va="top",
        fontsize=10,
...
def plot_length_histograms_shared_forced(
    ax_len, ax_ratio, out, len3, len2, bins=30, kde=True, bins_len=None, bins_rat=None
):
    """
    Right column:
    ...
    if bins_len is None:
        all_len = (
            ...
            if (forced["count"] or shared["count"])
            else np.array([])
        )
    ...
    if shared["count"]:
        _hist_kde(
            ax_ratio, 1 / shared["ratio"], bins_rat, color="C1", label="3D/2D (shared)", kde=kde
        )
    if forced["count"]:
        _hist_kde(
            ax_ratio, 1 / forced["ratio"], bins_rat, color="C2", label="3D/2D (forced)", kde=kde
        )
